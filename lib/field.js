  ...
  vf.at = function(obj) {
    ...
    if (obj.is_navigated) {
        ...
        for (var i=0; i<obj.navigation.length; i++) {
            if (frame%150 == 0 && obj.id == 'grownup') console.log(vec+"", dir, obj.navigation[i]);
...
function random_turns(interval, angle1, angle2){
  ...
  return field('effect', function(obj) {
    if (obj.is_dynamic && mod(frame, interval) == 0) {
...
function random_turns_with_pauses(interval, pause, pause_width, angle1, angle2){
  ...
  vf.at = function(obj) {
    if (obj.is_dynamic) {
      ...
      if (mod(frame, interval+this.pause) > interval) {
        obj.velocity = xy(0, 0);
      }
      if (mod(frame, interval+this.pause) == 0) {
